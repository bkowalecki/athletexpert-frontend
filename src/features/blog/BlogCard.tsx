...
const BlogCard: React.FC<BlogCardProps> = ({
  ...
  return (
    ...
    >
      {/* Bookmark Save Button (Floating Tab, top-right) */}
      ...
      {/* Pin Button (Floating Tab, top-left, profile only) */}
      {variant === "profile" && (
      ...
      <div className={`blog-card-content blog-card-content-${variant}`}>
        <h4 className={`blog-card-title blog-card-title-${variant}`}>
          {title}
        </h4>
        {publishedDate && (
